...
import { useLoading, useRequest, useResponseData } from "../lib";
import { AppDispatch } from "@/apps/store";
import { useAppDispatch } from "@/apps/hooks/app.hooks";
import { fetchUnits } from "../core/actions";
import { setUnitReset } from "../core/reducers/unit.reducer";
...
export const List = () => {
  ...
  useEffect(() => {
    return () => {
      dispatch(setUnitReset());
    };
  }, []);

  const unitList = useResponseData();
  const loading = useLoading()
  return (
    <LvCard>
      <div className="flex justify-between items-center w-full">
        <div className="text-[16px] font-semibold text-black">Unités</div>
