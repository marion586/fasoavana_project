...
import { setRoleRequest, setRoleResetRequest } from "../../core/reducers/role.reducer";

type Props = {
  className?: string;
  title?: string;
  tableProps: PropsWithChildren<HeaderProps<RoleModelItem>>;
};
export const CustomHeader: FC<Props> = ({ tableProps, className, title }) => {
  ...
  const sortColumn = () => {
    ...
    if (!isSelectedForSorting) {
      // enable Sort asc
      dispatch(
        setRoleRequest({ ...request, page: 1, sort: id, order: "ASC" })
      );
      return;
    }
    ...
    if (isSelectedForSorting && order !== undefined) {
      if (order === "ASC") {
        // enable sort desc
        dispatch(
          setRoleRequest({ ...request, page: 1, sort: id, order: "DESC" })
        );
        return;
      }

      // disable Sort
      dispatch(setRoleResetRequest());
    }
  };
